<div class="p-4 flex flex-col gap-4 h-full table-scroll-page">
    ...
    <div class="rounded-xl overflow-hidden border border-[rgba(9,20,59,0.16)]">
        <div style="background:#09143B;" class="px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-2">
                ...
                <span style="color:white;" class="text-sm font-medium">Estatísticas de Veículos</span>
            </div>
    ...
    <style>
        .veh-row-hover td, .veh-row-hover { background: #E4E2DF !important; }
    </style>
        ...
        <table style="border-collapse:collapse;width:100%;min-width:{{ count($diasLaborais) * 100 + 280 }}px;">
            ...
            <thead style="position:sticky;top:0;z-index:20;">
                <tr style="position:sticky;top:0;z-index:20;">
                    <th style="position:sticky;left:0;top:0;z-index:12;background:#09143B;color:white;font-size:0.75rem;font-weight:700;padding:10px 14px;text-align:left;white-space:nowrap;min-width:220px;border-right:2px solid rgba(255,255,255,0.12);">
                        Veículo
                    </th>
            ...
            <tbody>
                ...
                <tr style="background:{{ $rowBg }} !important;{{ !$veh->activo ? 'opacity:0.5;' : '' }}"
                    onmouseover="this.classList.add('veh-row-hover')"
                    onmouseout="this.classList.remove('veh-row-hover')">
                    {{-- Matrícula do veículo --}}
                    <td style="position:sticky;left:0;z-index:10;background:{{ $rowBg }} !important;padding:8px 14px;border-right:2px solid rgba(9,20,59,0.10);min-width:220px;border-bottom:1px solid rgba(9,20,59,0.06);">
                        <div style="font-size:12px;font-weight:700;color:#09143B;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px;" title="{{ $veh->matricula }} — {{ $veh->marca }} {{ $veh->modelo }}">
                            {{ $veh->matricula }}
                        </div>
                        @if($veh->marca || $veh->modelo)
                        <div style="font-size:10px;color:#7A7775;margin-top:1px;">{{ $veh->marca }} {{ $veh->modelo }}</div>
                        @endif
                    </td>
                    ...
                    {{-- Total dias --}}
                    <td style="position:sticky;right:0;z-index:10;background:{{ $rowBg }} !important;text-align:center;padding:6px 10px;border-left:2px solid rgba(9,20,59,0.14);border-bottom:1px solid rgba(9,20,59,0.06);font-size:13px;font-weight:800;color:#09143B;">
