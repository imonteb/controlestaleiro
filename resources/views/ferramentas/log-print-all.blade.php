...
<head>
    <meta charset="UTF-8">
    <title>RELATÓRIO DE VERIFICAÇÃO — Livro Completo</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 0;
...
<body>
    ...
    @php
        $groupedLogs = $logs->groupBy('folha_id');
    @endphp

    @for ($f = 1; $f <= $maxFolha; $f++)
        @php
            $currentLogs = $groupedLogs->get($f) ?? collect();
            $firstInPage = $currentLogs->first();
        @endphp
        <div class="page">
        ...
        {{-- ID Bar: Unidade --}}
        <div class="id-unidade">{{ $firstInPage?->unidade ?? 'C016' }}</div>
        ...
        <div class="obs-overlay">
            {{ $currentLogs->pluck('observacoes')->filter()->unique()->join('; ') }}
        </div>

        {{-- Registo number badge --}}
        <div class="registo-overlay">
            Registo: {{ $firstInPage?->num_registo_verificacao ?? 'N/A' }}
        </div>

        {{-- Print info --}}
        <div class="footer-overlay">
            <span>Impresso em: {{ now()->format('d/m/Y H:i') }}</span>
            <span>Pág. {{ $f }} de {{ $maxFolha }}</span>
        </div>
        </div>{{-- /.page --}}
